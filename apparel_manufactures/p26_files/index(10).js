...
var __async = (__this, __arguments, generator) => {
  ...
  const VALID_LIST_URL = "https://scm.alibaba.com/luyou/shipto/list.htm?action=list";
  const VALID_ADD_URL = "https://scm.alibaba.com/luyou/shipto/list.htm?action=add";
    ...
    if (null != t2) {
      ...
      try {
        if (i2 = (t2 = t2.call(r2)).next, 0 === l2) {
          ...
          for (; !(f2 = (e2 = i2.call(t2)).done) && (a.push(e2.value), a.length !== l2); f2 = true)
            ;
      } catch (r3) {
        o = true, n2 = r3;
      } finally {
  ...
  function ShipTo({ localData, onVisibleChange, i18n: propsI18N }) {
    ...
    const onSave = () => __async(this, null, function* () {
      ...
      if (selectAddressId === -1 && !params.localCountry && !params.localZipCode)
        return;
      ...
      if (loginRes) {
        ...
        addressList.forEach((item) => {
          if (item.selected)
            setSelectAddressId(item.appAddressId);
          if (item.isDefault)
            setDefaultAddressId(item.appAddressId);
        });
      }
      setLoading(false);
    });
