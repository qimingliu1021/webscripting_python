...
var __async = (__this, __arguments, generator) => {
  ...
  function transferDomain(domain, customHost) {
    const host = customHost || location && location.host;
    if (host) {
  ...
  */
  ...
  function parseImage(img, size, tpsSize) {
    ...
    if (/\/\/s\.alicdn\.com\/@sc0[1234]/.test(url)) {
      ...
      realSize = tpsSize || size;
    } else if (/\/\/s\.alicdn\.com\/@ossgw/.test(url)) {
      if (/x-oss-process/.test(url)) {
        return url;
      }
      const finalSize = size && tpsSize ? Math.min(size, tpsSize) : size || tpsSize;
  ...
  const Level3Panel = ({
    ...
    if (!currentLevel2 || !finalData || (finalData == null ? void 0 : finalData.length) === 0)
