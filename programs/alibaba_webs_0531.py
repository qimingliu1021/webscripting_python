...
for name, url in manufacturers_data:
    ...
    data = {
        ...
        'Main Markets':                 soup.find(string='Main markets').find_next('strong').text.strip() if soup.find(string='Main markets') else 'N/A',
...
# Write to CSV
with open('manufacturers_data.csv', 'w', newline='', encoding='utf-8') as csvfile:
